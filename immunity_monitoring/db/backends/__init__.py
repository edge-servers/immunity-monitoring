...
if not TIMESERIES_DB:
    TIMESERIES_DB = {
        ...
        'USER': getattr(settings, 'INFLUXDB_USER', 'immunity'),
        'PASSWORD': getattr(settings, 'INFLUXDB_PASSWORD', 'immunity'),
        'NAME': getattr(settings, 'INFLUXDB_DATABASE', 'immunity2'),
    ...
    logger.warning(
        ...
        'https://github.com/edge-servers/immunity-monitoring#setup-integrate-in-an-existing-django-project'
    )
...
def load_backend_module(backend_name=TIMESERIES_DB['BACKEND'], module=None):
    ...
    try:
        ...
        if backend_name not in [
            f'immunity_monitoring.db.backends.{b}' for b in builtin_backends
        ]:
            raise ImproperlyConfigured(
                f"{backend_name} isn't an available database backend.\n"
                "Try using 'immunity_monitoring.db.backends.XXX', where XXX is one of:\n"
                f"{builtin_backends}"
            ) from e
